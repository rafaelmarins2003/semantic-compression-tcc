...
from src.data.manipulation.llm.preprocess import (
    DEFAULT_MODELS,
    _parse_models,
...
def test_build_prompt_splits_system_and_user():
    system_prompt, user_prompt = build_prompt(
        "Cliente envia pedido. Financeiro aprova pagamento."
    )

    assert "<input>" in user_prompt
    assert "Cliente envia pedido. Financeiro aprova pagamento." in user_prompt
    assert "<input>" not in system_prompt
    assert "---PROCESSO---" in system_prompt
    assert "---FIM_PROCESSO---" in system_prompt
...
def test_pending_samples_skips_successful_generation(tmp_path):
    ...
    db.create_generation("s1", "preprocess", status="succeeded", output_text="done")
    db.create_generation("s2", "preprocess", status="failed", error="retry later")

    rows = pending_samples(db, stage="preprocess", limit=10, source="handbook", split="sft")
...
def test_pending_samples_respects_limit(tmp_path):
    ...
    rows = pending_samples(db, stage="preprocess", limit=1)
...
def test_default_model_fallback_order():
    assert DEFAULT_MODELS[0] == "kimi-k2.6:cloud"
