...
from src.data.llm.run_generate_json import (
...
def test_build_prompt_uses_preprocess_output():
    system_prompt, user_prompt = build_prompt("PROCESS: Approval\nACTORS:\n- Manager")

    assert system_prompt == ""
    assert "Return valid JSON" in user_prompt
    assert "<input_preprocess>" in user_prompt
    assert "PROCESS: Approval" in user_prompt


def test_generator_prompt_declares_output_language():
    """ADR 0001 / spec 004 AC-6: o prompt fixa o idioma em vez de herdá-lo da entrada."""
    _, user_prompt = build_prompt("PROCESS: Approval")

    assert "<language>" in user_prompt
    assert "in English" in user_prompt
    assert "Never mirror the input language" in user_prompt
