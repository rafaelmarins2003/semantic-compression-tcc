...
Examples:
    uv run python -m src.data.deterministic.run_dsl_to_xml
    uv run python -m src.data.deterministic.run_dsl_to_xml --limit 20
    uv run python -m src.data.deterministic.run_dsl_to_xml --retry-failed
"""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        help=(f"Per-sample wall-clock budget in seconds (default {DEFAULT_TIMEOUT_SECONDS})."),
    )
    return p.parse_args()
