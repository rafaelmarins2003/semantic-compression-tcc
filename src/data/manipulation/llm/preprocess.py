...
DEFAULT_PROMPT_VERSION = "preprocess_process_v1"
...
def pending_samples(
    ...
    *,
    ...
    """Return samples that need a generation attempt for a stage."""
    sql = """
        SELECT s.*
        FROM samples s
        ...
        LIMIT ?
    """
    rows = db.query(sql, (source, source, split, split, stage, stage, limit))
...
def run_preprocessing(
    *,
    ...
    with Database() as db:
        rows = pending_samples(db, stage=stage, limit=limit, source=source, split=split)
        if not rows:
            if execute:
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as pool:
            ...
            for future in as_completed(future_to_ctx):
                ...
                for attempt in result:
                    ...
                    detail = (
                        "" if attempt["status"] == "succeeded" else f": {attempt['error']}"
                    )
                    print(
