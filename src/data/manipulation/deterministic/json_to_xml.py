...
def _emit_process(root: etree._Element, graph: ProcessGraph, ids: XmlIds) -> str:
    ...
    flow_ids = {edge.id: ids.new("Flow", edge.id) for edge in graph.edges}
...
def _emit_node(
    ...
    for pred in graph.preds.get(node.id, []):
        ...
        if edge is not None:
            etree.SubElement(element, _bpmn("incoming")).text = flow_ids[edge.id]

    for succ in graph.succs.get(node.id, []):
        edge = graph.edge_map.get((node.id, succ))
        if edge is not None:
            etree.SubElement(element, _bpmn("outgoing")).text = flow_ids[edge.id]
...
def _emit_flow(
    ...
    attrs = {"id": flow_ids[edge.id], "sourceRef": source, "targetRef": target}
    if edge.label:
