...
Examples:
    uv run python -m src.data.manipulation.deterministic.run_json_to_dsl
    uv run python -m src.data.manipulation.deterministic.run_json_to_dsl --limit 20
    uv run python -m src.data.manipulation.deterministic.run_json_to_dsl --retry-failed
    uv run python -m src.data.manipulation.deterministic.run_json_to_dsl \\
...
DEFAULT_TRANSPILER_VERSION = "json_to_dsl_v8"
...
def pending_rows(
    ...
    *,
    ...
    sql = f"""
        ...
        LIMIT ?
    """
    rows = db.query(sql, (transpiler_version, limit))
...
def run_batch(
    *,
    ...
    with Database() as db:
        ...
        total = len(rows)
        if total == 0:
            print("[info] no pending rows for transpiler_version="
...
def print_summary(summary: dict, transpiler_version: str) -> None:
    ...
    print("  uv run python -c \"from src.data.db import Database; db=Database(); "
